...
class Model_Photo_Gateway_PDO {
	...
	public function getPhotoById($id) {
		...
		$s = $this->db->prepare("SELECT *, date_format(date_publish, '%Y-%m-%d %H:%i:%s') AS date_publish, date_format(date_publish, '%Y%m%d%H%i%s') AS datenum, (SELECT count(comment_id) FROM cel_photo_comments WHERE photo_id = :photo_id) AS comment_count FROM cel_photo_photos WHERE photo_id = :photo_id");
	...
	public function findByTag($tag) {
		...
		$s = $this->db->prepare("SELECT *, date_format(date_publish, '%Y-%m-%d %H:%i:%s') AS date_publish, date_format(date_publish, '%Y%m%d%H%i%s') AS datenum, (SELECT count(comment_id) FROM cel_photo_comments WHERE photo_id = cel_photo_photos.photo_id) AS comment_count FROM cel_photo_photos WHERE FIND_IN_SET(:tag, tags)");
		$s->execute(array(':tag'=> $tag));
		return $s->fetchAll(PDO::FETCH_ASSOC);
